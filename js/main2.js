// TRKA v2 — spajanje svega: ekrani, kalibracija, petlja igre.
...
// v2: plitak i spor čučanj u prvoj verziji nije bio detektovan jer se osnova
// polako spuštala zajedno sa telom. Niži prag + osnova se ne pomera dok si sagnut.
analyzer.tune = {
  jumpOn: 0.16, jumpOff: 0.07,
  crouchOn: 0.30, crouchOff: 0.16,
  adapt: 0.35, adaptGate: 0.12,
  graceMs: 380
};
...
let calibTime = 0;
let toastT = 0;
let best = +(localStorage.getItem('ka2_best') || 0);
...
function gameOver() {
  ...
  localStorage.setItem('ka2_best', String(best));
...
function updateHud(input) {
  ...
  if (el.dataset.n !== String(game.lives)) {
    const n = game.lives;
    let hearts;
    if (n <= 0) hearts = '<i class="off">♥</i>';
    else if (n > 6) hearts = '<i>♥</i><b class="mult">×' + n + '</b>';
    else { hearts = ''; for (let i = 0; i < n; i++) hearts += '<i>♥</i>'; }
    el.innerHTML = hearts;
...
// Debug pristup iz konzole: KA2.game, KA2.tracker, KA2.analyzer
window.KA2 = {
